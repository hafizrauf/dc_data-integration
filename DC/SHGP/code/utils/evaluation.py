...
def revised_rand_index(actual, pred):
    # Convert to integer type if it's numpy array or convert tensor to numpy array
    ...
    A = np.c_[(actual, pred)]
    tp = sum(comb(np.bincount(A[A[:, 0] == i, 1]), 2).sum() for i in set(actual))
    fp = tp_plus_fp - tp
    fn = tp_plus_fn - tp
...
def cluster_acc2(y_true, y_pred):
    # Check if the input is a PyTorch tensor and convert to numpy if needed
