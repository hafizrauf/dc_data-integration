...
from utils.cluster import evaluate_clustering
...
def train():
  ...
  #      kmeans = KMeans(n_clusters=num_cluster,random_state=2)
  #      y_pred = kmeans.fit_predict(embd[target_type].detach().numpy())
  #      print("epoch # ", epoch)
  #      sil= silhouette_score(embd[target_type].detach().numpy(), y_pred)
...
#    evaluate(target_embd, idx_train, idx_val, idx_test, true_label, num_class, isTest=True)
    evaluate_clustering(target_embd, true_label, num_cluster)
